...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      successful: filteredPayments.filter(p => p.status === 'completed' || p.status === 'succeeded').length,
      failed: filteredPayments.filter(p => p.status === 'failed').length,
      successRate: filteredPayments.length > 0 ? 
        `${Math.round((filteredPayments.filter(p => p.status === 'completed' || p.status === 'succeeded').length / filteredPayments.length) * 100)}%` : 
        '0%'
    };
